...
double EuclideanDistance(const std::vector<double>& v1, const std::vector<double>& v2)
{
    if (v1.size() != v2.size())
    {
        ROS_ERROR("Euclidean distance computation failed: the two vectors don't have the same size!");
        return 1;
    }
    double result(0);
...
ObjectRecognizer::ObjectRecognizer(const std::vector<ObjectFeatures>& dataset) : dataset_(dataset) {}
...
void ObjectRecognizer::Recognize(const Object& object, std::string* name, double* confidence)
{
    ...
    perception::ExtractSizeFeatures(object, features_input);
    double distance; // Placeholder
    ...
    for (size_t i=0 ; i < dataset_.size() ; i++)
    {
        ...
        double distance = EuclideanDistance(features_input->values, dataset_[i].values);
